...
int i2c_tsp_sensor_read(u16 reg, u8 *read_val, unsigned int len )
{
	...
	{
		printk(KERN_INFO "[TSP][I2C] read addr[0] = 0x%x, addr[1] = 0x%x\n", data[0], data[1]);
	}
#endif
	...
	if (err >= 0) 
	{
		...
		if(g_i2c_debugging_enable == 1)
		{
			printk(KERN_INFO "[TSP][I2C] read data = ");
			for(i=0 ; i<len ; i++)
			{
				printk(KERN_INFO "%d(0x%x), ", msg->buf[i],  msg->buf[i]);
			}
			printk(KERN_INFO "// len = %d, rtn=%d\n",msg->len, err);
		}
#endif
	}
...
//		goto ERROR1;
	printk(KERN_INFO "[TSP] %s() - end : success\n", __FUNCTION__);	// ryun 20091126
	return 0;
...
int i2c_tsp_sensor_init(void)
{
	...
	{
		printk(KERN_INFO "TSP I2C Driver registration failed!\n");
		return ret;
	}
...
uint8_t read_boot_state(u8 *data)
{
	...
	if ( ret < 0 )
	{
		printk(KERN_INFO "[CHJ] %s,%d - Fail!!!! ret = %d\n", __func__, __LINE__, ret );
...
uint8_t boot_unlock(void)
{
	...
	printk(KERN_INFO "[CHJ] %s - start, %d\n", __func__, __LINE__ );
